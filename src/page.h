...
#ifndef CROWLIT_PAGE_H
#define CROWLIT_PAGE_H
 ...
 */
    struct tx_details
    {
        ...
        // public keys and lit amount of outputs
        ...
        get_mstch_map() const
        {
            ...
            const double& lit_amount = LIT_AMOUNT(fee);

            if (!input_key_imgs.empty())
            {
                mixin_str     = std::to_string(mixin_no);
                fee_str       = fmt::format("{:0.6f}", lit_amount);
                fee_short_str = fmt::format("{:0.3f}", lit_amount);
            }
            ...
            mstch::map txd_map {
                    ...
                    {"sum_inputs"        , lit_amount_to_str(lit_inputs , "{:0.6f}")},
                    {"sum_outputs"       , lit_amount_to_str(lit_outputs, "{:0.6f}")},
                    {"sum_inputs_short"  , lit_amount_to_str(lit_inputs , "{:0.3f}")},
                    {"sum_outputs_short" , lit_amount_to_str(lit_outputs, "{:0.3f}")},
    ...
    class page
    {
        ...
        // index page, so that we dont parse txs in each block
        // for each request.
    ...
    public:
                  ...
                  tx_context_cache(1000)
        {
            ...
            template_file["css_styles"]      = liteg::read(TMPL_CSS_STYLES);
            template_file["header"]          = liteg::read(TMPL_HEADER);
            template_file["footer"]          = get_footer();
            template_file["index"]          = get_full_page(liteg::read(TMPL_INDEX));
            template_file["mempool"]         = liteg::read(TMPL_MEMPOOL);
            template_file["altblocks"]       = get_full_page(liteg::read(TMPL_ALTBLOCKS));
            template_file["mempool_error"]   = liteg::read(TMPL_MEMPOOL_ERROR);
            template_file["mempool_full"]    = get_full_page(template_file["mempool"]);
            template_file["block"]           = get_full_page(liteg::read(TMPL_BLOCK));
            template_file["tx"]              = get_full_page(liteg::read(TMPL_TX));
            template_file["my_outputs"]      = get_full_page(liteg::read(TMPL_MY_OUTPUTS));
            template_file["rawtx"]           = get_full_page(liteg::read(TMPL_MY_RAWTX));
            template_file["checkrawtx"]      = get_full_page(liteg::read(TMPL_MY_CHECKRAWTX));
            template_file["pushrawtx"]       = get_full_page(liteg::read(TMPL_MY_PUSHRAWTX));
            template_file["rawkeyimgs"]      = get_full_page(liteg::read(TMPL_MY_RAWKEYIMGS));
            template_file["rawoutputkeys"]   = get_full_page(liteg::read(TMPL_MY_RAWOUTPUTKEYS));
            template_file["checkrawkeyimgs"] = get_full_page(liteg::read(TMPL_MY_CHECKRAWKEYIMGS));
            template_file["checkoutputkeys"] = get_full_page(liteg::read(TMPL_MY_CHECKRAWOUTPUTKEYS));
            template_file["address"]         = get_full_page(liteg::read(TMPL_ADDRESS));
            template_file["search_results"]  = get_full_page(liteg::read(TMPL_SEARCH_RESULTS));
            template_file["tx_details"]      = liteg::read(string(TMPL_PARIALS_DIR) + "/tx_details.html");
            template_file["tx_table_header"] = liteg::read(string(TMPL_PARIALS_DIR) + "/tx_table_header.html");
            template_file["tx_table_row"]    = liteg::read(string(TMPL_PARIALS_DIR) + "/tx_table_row.html");
        }
        ...
        index(uint64_t page_no = 0, bool refresh_page = false)
        {
            ...
            // index method, until deamon gives as the required result.
            std::future<json> network_info_ftr = std::async(std::launch::async, [&]
            {
                json j_info;

                get_bitlitas_network_info(j_info);
            ...
            mstch::map context {
                    ...
                    {"height"                   , height},
                    {"server_timestamp"         , liteg::timestamp_to_str_gm(local_copy_server_timestamp)},
            ...
            double blk_size_median = liteg::calc_median(blk_sizes.begin(), blk_sizes.end());
            ...
            if (CurrentBlockchainStatus::is_thread_running())
            {
                ...
                string emission_coinbase = lit_amount_to_str(current_values.coinbase, "{:0.3f}");
                string emission_fee      = lit_amount_to_str(current_values.fee, "{:0.3f}");
            ...
            // render the page
            return mstch::render(template_file["index"], context);
        }
        ...
        mempool(bool add_header_and_footer = false, uint64_t no_of_mempool_tx = 25)
        {
            ...
            for (size_t i = 0; i < no_of_mempool_tx; ++i)
            {
                ...
                txs.push_back(mstch::map {
                        ...
                        {"lit_inputs"      , mempool_tx.lit_inputs_str},
                        {"lit_outputs"     , mempool_tx.lit_outputs_str},
        ...
        show_block(uint64_t _blk_height)
        {
            ...
            string blk_timestamp = liteg::timestamp_to_str_gm(blk.timestamp);
            ...
            for (size_t i = 0; i < blk.tx_hashes.size(); ++i)
            {
                ...
                //string mixin_times_scale = liteg::timestamps_time_scale(mixin_timestamps,
                //                                                        server_timestamp);
            ...
            context["sum_fees"]
                    = liteg::lit_amount_to_str(sum_fees, "{:0.6f}", "0");

            // get lit in the block reward
            context["blk_reward"]
                    = liteg::lit_amount_to_str(txd_coinbase.lit_outputs - sum_fees, "{:0.6f}");
        ...
        show_block(string _blk_hash)
        {
            ...
            if (!liteg::parse_str_secret_key(_blk_hash, blk_hash))
            {
        ...
        show_tx(string tx_hash_str, uint16_t with_ring_signatures = 0)
        {
            ...
            if (!liteg::parse_str_secret_key(tx_hash_str, tx_hash))
            {
            ...
            if (!mcore->get_tx(tx_hash, tx))
            {
                ...
                if (!found_txs.empty())
                {
                    ...
                    blk_timestamp = liteg::timestamp_to_str_gm(tx_recieve_timestamp);
                        ...
                        bool tx_prove = false)
        {
            ...
            if (lit_address_str.empty())
            {
                return string("Bitlitas address not provided!");
            }
            ...
            // parse string representing given bitlitas address
            cryptonote::address_parse_info address_info;

            if (!liteg::parse_str_address(lit_address_str,  address_info, testnet))
            {
                cerr << "Cant parse string address: " << lit_address_str << endl;
                return string("Cant parse lit address: " + lit_address_str);
            }
            ...
            if (!liteg::parse_str_secret_key(viewkey_str, multiple_tx_secret_keys))
            {
...
//        if (!liteg::parse_str_secret_key(spend_key_str, prv_spend_key))
//        {
...
//        }
            ...
            if (tx_blk_found)
            {
                ...
                blk_timestamp = liteg::timestamp_to_str_gm(blk.timestamp);
            ...
            mstch::map context {
                    ...
                    {"lit_address"          , lit_address_str},
                    {"viewkey"              , viewkey_str_partial},
                    {"tx_pub_key"           , pod_to_hex(txd.pk)},
                    {"blk_height"           , tx_blk_height_str},
                    {"tx_size"              , fmt::format("{:0.4f}",
                                                          static_cast<double>(txd.size) / 1024.0)},
                    {"tx_fee"               , liteg::lit_amount_to_str(txd.fee, "{:0.12f}", true)},
                    {"blk_timestamp"        , blk_timestamp},
            ...
            string server_time_str = liteg::timestamp_to_str_gm(server_timestamp, "%F");
            ...
            for (pair<txout_to_key, uint64_t>& outp: txd.output_pub_keys)
            {
                ...
                // if someone had sent us some lit.
                ...
                outputs.push_back(mstch::map {
                        ...
                        {"amount"        , liteg::lit_amount_to_str(outp.second)},
            ...
            vector<txin_to_key> input_key_imgs = liteg::get_key_images(tx);

            // to hold sum of lit in matched mixins, those that
            // perfectly match mixin public key with outputs in mixn_tx.
            uint64_t sum_mixin_lit {0};
            ...
            for (const txin_to_key& in_key: input_key_imgs)
            {
                ...
                inputs.push_back(mstch::map{
                        ...
                        {"key_image_amount", liteg::lit_amount_to_str(in_key.amount)},
                ...
                for (const uint64_t& abs_offset: absolute_offsets)
                {
                    ...
                    public_key mixin_tx_pub_key
                            = liteg::get_tx_pub_key_from_received_outs(mixin_tx);
                    ...
                    output_pub_keys = liteg::get_ouputs_tuple(mixin_tx);
...
//                        }
                        ...
                        found_outputs.push_back(mstch::map {
                                ...
                                {"amount"          , liteg::lit_amount_to_str(amount)}
                        });
                        ...
                        if (mine_output)
                        {
                            ...
                            // increase sum_mixin_lit only when
...
//
//                        if (!liteg::generate_key_image(derivation,
...
//                                << endl;
            ...
            context["found_our_outputs"] = (sum_lit > 0);
            context["sum_lit"]           = liteg::lit_amount_to_str(sum_lit);

            context.emplace("inputs", inputs);

            context["show_inputs"]   = show_key_images;
            context["inputs_no"]     = static_cast<uint64_t>(inputs.size());
            context["sum_mixin_lit"] = liteg::lit_amount_to_str(
                    sum_mixin_lit, "{:0.12f}", false);
            ...
            if (sum_mixin_lit > (sum_lit + txd.fee)
                && no_of_matched_mixins == inputs.size())
            {
                //                  (outcoming    - incoming) - fee
                possible_spending = (sum_mixin_lit - sum_lit) - txd.fee;
            }

            context["possible_spending"] = liteg::lit_amount_to_str(
                   ...
                   string domain)
        {
            ...
            return show_my_outputs(tx_hash_str, lit_address_str,
        ...
        show_checkrawtx(string raw_tx_data, string action)
        {
            ...
            string data_prefix = liteg::make_printable(decoded_raw_tx_data.substr(0, magiclen));
                ...
                if (r)
                {
                    ...
                    for (const ::tools::wallet2::tx_construction_data& tx_cd: exported_txs.txes)
                    {
                        ...
                        mstch::map tx_cd_data {
                                ...
                                {"change_amount"      , liteg::lit_amount_to_str(tx_change.amount)},
                                {"has_payment_id"     , (payment_id  != null_hash)},
                                {"has_payment_id8"    , (payment_id8 != null_hash8)},
                                {"payment_id"         , pid_str},
                                {"payment_id8"        , pid8_str},
                        ...
                        for (const tx_destination_entry& a_dest: tx_cd.splitted_dsts)
                        {
                            mstch::map dest_info {
                                    ...
                                    {"dest_amount"   , liteg::lit_amount_to_str(a_dest.amount)}
                            };
                        ...
                        for (size_t i = 0; i < no_of_sources; ++i)
                        {
                            ...
                            mstch::map single_dest_source {
                                    {"output_amount"              , liteg::lit_amount_to_str(tx_source.amount)},
                        ...
                        tx_cd_data.insert({"sum_outputs_amounts" ,
                                           liteg::lit_amount_to_str(sum_outputs_amounts)});
                        ...
                        tx_cd_data["min_mix_time"]     = liteg::timestamp_to_str_gm(min_mix_timestamp);
                        tx_cd_data["max_mix_time"]     = liteg::timestamp_to_str_gm(max_mix_timestamp);
            ...
            else
            {
                ...
                if (strncmp(decoded_raw_tx_data.c_str(), SIGNED_TX_PREFIX, magiclen) != 0)
                {
                    ...
                    // used in mybitlitas and openbitlitas projects.
                ...
                for (tools::wallet2::pending_tx& ptx: ptxs)
                {
                    ...
                    if (ptx.construction_data.change_dts.amount > 0)
                    {
                        destination_addresses.push_back(
                                mstch::map {
                                        ...
                                        {"dest_amount"    ,
                                                liteg::lit_amount_to_str(ptx.construction_data.change_dts.amount)},
                                        {"is_this_change" , true}
                                }
                        );
                    ...
                    tx_context["outputs_lit_sum"] = liteg::lit_amount_to_str(outputs_lit_sum);
                    ...
                    for (size_t i = 0; i < outputs.size(); ++i)
                    {
                        ...
                        if (parse_amount(output_amount, out_amount_str))
                        {
                            if (output_amount == 0)
                            {
                                out_amount_str = liteg::lit_amount_to_str(real_ammounts.at(i));
                            }
                        }
                    }
                    ...
                    for (const tx_source_entry&  tx_source: ptx.construction_data.sources)
                    {
                        ...
                        inputs_lit_sum += tx_source.amount;
                    }

                    // mark that we have signed tx data for use in mstch
                    tx_context["have_raw_tx"] = true;

                    // provide total mount of inputs lit
                    tx_context["inputs_lit_sum"] = liteg::lit_amount_to_str(inputs_lit_sum);
                    ...
                    for (mstch::node& input_node: inputs)
                    {
                        ...
                        amount = liteg::lit_amount_to_str(real_amounts.at(input_idx));
        ...
        show_pushrawtx(string raw_tx_data, string action)
        {
            ...
            // if failed, treat raw_tx_data as base64 encoding of signed_bitlitas_tx
            else
            {
                string decoded_raw_tx_data = epee::string_encoding::base64_decode(raw_tx_data);
        ...
        show_checkrawkeyimgs(string raw_data, string viewkey_str)
        {
            ...
            if (!liteg::parse_str_secret_key(viewkey_str, prv_view_key))
            {
            ...
            string data_prefix = liteg::make_printable(decoded_raw_data.substr(0, magiclen));
            ...
            decoded_raw_data = liteg::decrypt(
            ...
            // get lit address stored in this key image file
            const account_public_address* lit_address =
                    reinterpret_cast<const account_public_address*>(
                            decoded_raw_data.data());

            address_parse_info address_info {*lit_address, false};


            context.insert({"address"        , REMOVE_HASH_BRAKETS(
                    liteg::print_address(address_info, testnet))});
            context.insert({"viewkey"        , REMOVE_HASH_BRAKETS(
                    fmt::format("{:s}", prv_view_key))});
            context.insert({"has_total_lit"  , false});
            context.insert({"total_lit"      , string{}});
        ...
        show_checkcheckrawoutput(string raw_data, string viewkey_str)
        {
            ...
            for (const tools::wallet2::transfer_details& td: outputs)
            {
                ...
                if (td.is_rct())
                {
                    ...
                    public_key tx_pub_key = liteg::get_tx_pub_key_from_received_outs(tx);
                ...
                mstch::map output_info {
                        ...
                        {"amount"              , liteg::lit_amount_to_str(lit_amount)},
                        {"tx_hash"             , REMOVE_HASH_BRAKETS(fmt::format("{:s}", td.m_txid))},
                        {"timestamp"           , liteg::timestamp_to_str_gm(blk_timestamp)},
            ...
            if (total_lit > 0)
            {
                context["has_total_lit"] = true;
                context["total_lit"] = liteg::lit_amount_to_str(total_lit);
            }
        ...
        search(string search_text)
        {
            ...
            // check if bitlitas address is given based on its length
            ...
            if (search_str_length == 95)
            {
                ...
                if (!liteg::parse_str_address(search_text, address_info, testnet_addr))
                {
            ...
            // check if integrated bitlitas address is given based on its length
        ...
        show_address_details(const address_parse_info& address_info, bool testnet = false)
        {
            ...
            string address_str      = liteg::print_address(address_info, testnet);
            ...
            mstch::map context {
                    {"lit_address"        , REMOVE_HASH_BRAKETS(address_str)},
            ...
            for (const pair<string, vector<string>>& found_txs: all_possible_tx_hashes)
            {
                ...
                if (!found_txs.second.empty())
                {
                    ...
                    for (const string& tx_hash: found_txs.second)
                    {
                        ...
                        txd_map.insert({"timestamp", liteg::timestamp_to_str_gm(blk_timestamp)});
        ...
        json_transaction(string tx_hash_str)
        {
            ...
            string blk_timestamp_utc = liteg::timestamp_to_str_gm(tx_timestamp);
        ...
        json_rawtransaction(string tx_hash_str)
        {
            ...
            // get raw tx json as in bitlitas
        ...
        json_block(string block_no_or_hash)
        {
            ...
            else if (block_no_or_hash.length() == 64)
            {
                ...
                if (!liteg::parse_str_secret_key(block_no_or_hash, blk_hash))
                {
            ...
            j_data = json {
                    ...
                    {"timestamp_utc" , liteg::timestamp_to_str_gm(blk.timestamp)},
                    {"block_height"  , block_height},
                    {"size"          , blk_size},
                    {"txs"           , j_txs},
     ...
     */
                     ...
                     bool tx_prove = false)
        {
            ...
            {
                j_response["status"]  = "error";
                j_response["message"] = "Bitlitas address not provided";
                return j_response;
            }
            ...
            if (!liteg::parse_str_address(address_str,  address_info, testnet))
            {
                j_response["status"]  = "error";
                j_response["message"] = "Cant parse bitlitas address: " + address_str;
                return j_response;
        ...
        json_networkinfo()
        {
            ...
            if (!get_bitlitas_network_info(j_info))
            {
                j_response["status"]  = "error";
                j_response["message"] = "Cant get bitlitas network info";
                return j_response;
            }
        ...
        */
        ...
        json_emission()
        {
            ...
            else
            {
                ...
                string emission_blk_no   = std::to_string(current_values.blk_no - 1);
                string emission_coinbase = lit_amount_to_str(current_values.coinbase, "{:0.3f}");
                string emission_fee      = lit_amount_to_str(current_values.fee, "{:0.3f}", false);
        ...
        json_version()
        {
            ...
            j_data = json {
                    ...
                    {"bitlitas_version_full" , string {BITLITAS_VERSION_FULL}},
    ...
    private:
        ...
        get_tx_json(const transaction& tx, const tx_details& txd)
        {
            ...
            json j_tx {
                    ...
                    {"mixin"       , txd.mixin_no},
                    {"tx_size"     , txd.size},
                    {"lit_outputs" , txd.lit_outputs},
                    {"lit_inputs"  , txd.lit_inputs},
        ...
        construct_tx_context(transaction tx, uint16_t with_ring_signatures = 0)
        {
            ...
            mstch::map context {
                    ...
                    {"tx_size"               , fmt::format("{:0.4f}",
                                                           static_cast<double>(txd.size) / 1024.0)},
                    {"tx_fee"                , liteg::lit_amount_to_str(txd.fee, "{:0.12f}", false)},
                    {"tx_version"            , static_cast<uint64_t>(txd.version)},
                    {"blk_timestamp"         , blk_timestamp},
            ...
            for (const txin_to_key &in_key: txd.input_key_imgs)
            {
                ...
                inputs.push_back(mstch::map {
                        ...
                        {"amount"       , liteg::lit_amount_to_str(in_key.amount)},
                ...
                for (const uint64_t& i: absolute_offsets)
                {
                    ...
                    if (detailed_view)
                    {
                        ...
                        mixins.push_back(mstch::map {
                                ...
                                {"mix_timestamp",  liteg::timestamp_to_str_gm(blk.timestamp)},
            ...
            if (detailed_view)
            {
                ...
                context["min_mix_time"]     = liteg::timestamp_to_str_gm(min_mix_timestamp);
                context["max_mix_time"]     = liteg::timestamp_to_str_gm(max_mix_timestamp);
            ...
            context["inputs_lit_sum_not_zero"]  = (inputs_lit_sum > 0);
            context["inputs_lit_sum"]           = liteg::lit_amount_to_str(inputs_lit_sum);
            ...
            context["outputs_lit_sum"] = liteg::lit_amount_to_str(outputs_lit_sum);
        ...
        )
        {
            ...
            {
                // get mixins in time scale for visual representation
                pair<string, double> mixin_times_scale = liteg::timestamps_time_scale(
                        mixn_timestamps,
                        max_mix_timestamp,
                        170,
                        min_mix_timestamp);
                       ...
                       uint64_t bc_height = 0)
        {
            ...
            txd.pk = liteg::get_tx_pub_key_from_received_outs(tx);
            txd.additional_pks = cryptonote::get_additional_tx_pub_keys_from_extra(tx);

            // sum lit in inputs and ouputs in the given tx
            const array<uint64_t, 4>& sum_data = summary_of_in_out_rct(
                    tx, txd.output_pub_keys, txd.input_key_imgs);

            txd.lit_outputs       = sum_data[0];
            txd.lit_inputs        = sum_data[1];
        ...
        bool
        get_bitlitas_network_info(json& j_info)
        {
        ...
        get_footer()
        {
            ...
            string footer_html = mstch::render(liteg::read(TMPL_FOOTER), footer_context);
