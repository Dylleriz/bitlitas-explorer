...
/**
* Parse bitlitas address in a string form into
...
/**
* Return string representation of bitlitas address
*/
string
...
{
    // default path to bitlitas folder
    // on linux this is /home/<username>/.bitlitas
    string default_bitlitas_dir = tools::get_default_data_dir();

    if (testnet)
        default_bitlitas_dir += "/testnet";


    // the default folder of the lmdb blockchain database
    // is therefore as follows
    return default_bitlitas_dir + string("/lmdb");
}
...
{
    // the default folder of the lmdb blockchain database
    string default_lmdb_dir   = liteg::get_default_lmdb_folder(testnet);
    ...
    blockchain_path = liteg::remove_trailing_path_separator(blockchain_path);
...
{
    pair<uint64_t, uint64_t> sum_lit {0, 0};
    ...
    {
        sum_lit.first += vout["amount"].get<uint64_t>();
        ++sum_lit.second;
    }
...
{
    pair<uint64_t, uint64_t> sum_lit {0ULL, 0ULL};
        ...
        vector<txin_to_key>& input_key_imgs)
{
    ...
    uint64_t lit_outputs       {0};
    uint64_t lit_inputs        {0};
    uint64_t mixin_no          {0};
    ...
    for (size_t i = 0; i < input_no; ++i)
    {
        ...
        lit_inputs += tx_in_to_key.amount;
    ...
    return {lit_outputs, lit_inputs, mixin_no, num_nonrct_inputs};
};
...
summary_of_in_out_rct(const json& _json)
{
    ...
    for (const json& vout: _json["vout"])
    {
        lit_outputs += vout["amount"].get<uint64_t>();
    }
    ...
    return {lit_outputs, lit_inputs, no_outputs, no_inputs, mixin_no, num_nonrct_inputs};
};
...
sum_money_in_inputs(const string& json_str)
{
    ...
    for (json& vin: j["vin"])
    {
        sum_lit.first += vin["key"]["amount"].get<uint64_t>();
        ++sum_lit.second;
    }
...
{
    array<uint64_t, 2> sum_lit;

    sum_lit[0] = sum_money_in_inputs(tx);
    sum_lit[1] = sum_money_in_outputs(tx);
