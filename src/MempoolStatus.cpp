...
MempoolStatus::read_mempool()
{
    ...
    for (size_t i = 0; i < mempool_tx_info.size(); ++i)
    {
        ...
        last_tx.fee_str         = liteg::lit_amount_to_str(_tx_info.fee, "{:0.3f}", false);
        last_tx.lit_inputs_str  = liteg::lit_amount_to_str(last_tx.sum_inputs , "{:0.3f}");
        last_tx.lit_outputs_str = liteg::lit_amount_to_str(last_tx.sum_outputs, "{:0.3f}");
        last_tx.timestamp_str   = liteg::timestamp_to_str_gm(_tx_info.receive_time);
...
bf::path MempoolStatus::blockchain_path {"/home/mwo/.bitlitas/lmdb"};
...
liteg::MicroCore*  MempoolStatus::mcore {nullptr};
