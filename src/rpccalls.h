...
#ifndef CROWLIT_RPCCALLS_H
#define CROWLIT_RPCCALLS_H
...
{
// declare struct in bitlitas's cryptonote namespace.
// bitlitas should provide definition for this,
...
public:
    ...
    get_alt_blocks(vector<string>& alt_blocks_hashes)
    {
        ...
        if (r)
        {
            ...
            if (!err.empty())
            {
                cerr << "Error connecting to Bitlitas deamon due to "
                     << err << endl;
                return false;
            }
        }
        ...
        {
            cerr << "Error connecting to Bitlitas deamon at "
                 << deamon_url << endl;
            return false;
        }
