...
#ifndef LITEG01_TOOLS_H
#define LITEG01_TOOLS_H

#define PATH_SEPARARTOR '/'

#define LIT_AMOUNT(value) \
...
static
string
lit_amount_to_str(const uint64_t& lit_amount,
                  ...
                  bool zero_to_question_mark=true)
{
    ...
    {
        amount_str = fmt::format(_format, LIT_AMOUNT(lit_amount));
    }
    else
    {
        if (lit_amount > 0 && zero_to_question_mark == true)
        {
            amount_str = fmt::format(_format, LIT_AMOUNT(lit_amount));
        }
    }
