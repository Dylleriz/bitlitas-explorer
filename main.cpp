...
main(int ac, const char* av[])
{
    ...
    liteg::CmdLineOptions opts {ac, av};
    ...
    auto ssl_chain_file_opt            = opts.get_option<string>("ssl-chain-file");
    ...
    string ssl_crt_file;
    string ssl_key_file;
    string ssl_chain_file;


    // check if ssl enabled and files exist

    if (ssl_crt_file_opt and ssl_key_file_opt and ssl_chain_file_opt)
    {
        ...
        if (!boost::filesystem::exists(boost::filesystem::path(*ssl_chain_file_opt)))
        {
            cerr << "ssl_chain_file path: " << *ssl_chain_file_opt
                 << "does not exist!" << endl;
        ...
        ssl_chain_file = *ssl_chain_file_opt;
    ...
    if (!liteg::get_blockchain_path(bc_path_opt, blockchain_path, testnet))
    {
    ...
    liteg::MicroCore mcore;
    cryptonote::Blockchain* core_storage;

    // initialize mcore and core_storage
    if (!liteg::init_blockchain(blockchain_path.string(),
                               mcore, core_storage))
    {
    ...
    if (enable_emission_monitor == true)
    {
        ...
        liteg::CurrentBlockchainStatus::blockchain_path
                = blockchain_path;
        liteg::CurrentBlockchainStatus::testnet
                = testnet;
        liteg::CurrentBlockchainStatus::deamon_url
                = deamon_url;
        liteg::CurrentBlockchainStatus::set_blockchain_variables(
                &mcore, core_storage);
        ...
        liteg::CurrentBlockchainStatus::start_monitor_blockchain_thread();
    }


    liteg::MempoolStatus::blockchain_path
            = blockchain_path;
    liteg::MempoolStatus::testnet
            = testnet;
    liteg::MempoolStatus::deamon_url
            = deamon_url;
    liteg::MempoolStatus::set_blockchain_variables(
            &mcore, core_storage);
    ...
    // by tx searching threads that are launched for each user independently,
    // when they log back or create new account.
    liteg::MempoolStatus::mempool_refresh_time = mempool_refresh_time;
    liteg::MempoolStatus::start_mempool_status_thread();

    // create instance of page class which
    // contains logic for the website
    liteg::page litblocks(&mcore,
                          core_storage,
                          deamon_url,
                          testnet,
                          enable_pusher,
    ...
    ([&](const crow::request& req) {
        return crow::response(litblocks.index());
    });

    CROW_ROUTE(app, "/puslapis/<uint>")
    ([&](size_t page_no) {
        return litblocks.index(page_no);
    });

    CROW_ROUTE(app, "/blokas/<uint>")
    ([&](const crow::request& req, size_t block_height) {
        return crow::response(litblocks.show_block(block_height));
    });

    CROW_ROUTE(app, "/blokas/<string>")
    ([&](const crow::request& req, string block_hash) {
        return crow::response(litblocks.show_block(remove_bad_chars(block_hash)));
    });

    CROW_ROUTE(app, "/tx/<string>")
    ([&](const crow::request& req, string tx_hash) {
        return crow::response(litblocks.show_tx(remove_bad_chars(tx_hash)));
    });

    CROW_ROUTE(app, "/tx/<string>/<uint>")
    ([&](string tx_hash, uint16_t with_ring_signatures)
     {
        return litblocks.show_tx(remove_bad_chars(tx_hash), with_ring_signatures);
    });
    ...
    ([&](const crow::request& req)
     {
        ...
        map<std::string, std::string> post_body
                = liteg::parse_crow_post_data(req.body);

        if (post_body.count("lit_address") == 0
            || post_body.count("viewkey") == 0
            || post_body.count("tx_hash") == 0)
        {
            return string("lit address, viewkey or tx hash not provided");
        }

        string tx_hash     = remove_bad_chars(post_body["tx_hash"]);
        string lit_address = remove_bad_chars(post_body["lit_address"]);
        ...
        return litblocks.show_my_outputs(tx_hash, lit_address,
                                         viewkey, raw_tx_data,
                                         domain);
    });
    ...
    ([&](const crow::request& req, string tx_hash,
        string lit_address, string viewkey)
     {

        string domain = get_domain(req);

        return litblocks.show_my_outputs(remove_bad_chars(tx_hash),
                                         remove_bad_chars(lit_address),
    ...
    CROW_ROUTE(app, "/prove").methods("POST"_method)
        ([&](const crow::request& req) {
            ...
            if (post_body.count("litaddress") == 0
                || post_body.count("txprvkey") == 0
                || post_body.count("txhash") == 0)
            {
                return string("lit address, tx private key or "
            ...
            string lit_address = remove_bad_chars(post_body["litaddress"]);
            ...
            return litblocks.show_prove(tx_hash,
                                        lit_address,
                                        tx_prv_key,
                                        raw_tx_data,
                                        domain);
    ...
    ([&](const crow::request& req, string tx_hash,
         string lit_address, string tx_prv_key) {

        string domain = get_domain(req);

        return litblocks.show_prove(remove_bad_chars(tx_hash),
                                    remove_bad_chars(lit_address),
    ...
    if (enable_pusher)
    {
        ...
        ([&](const crow::request& req) {
            ...
            if (action == "check")
                return litblocks.show_checkrawtx(raw_tx_data, action);
            else if (action == "push")
                return litblocks.show_pushrawtx(raw_tx_data, action);
    ...
    if (enable_key_image_checker)
    {
        ...
        ([&](const crow::request& req) {
            return litblocks.show_rawkeyimgs();
        });
        ...
        ([&](const crow::request& req) {
            ...
            return litblocks.show_checkrawkeyimgs(raw_data, viewkey);
        });
    }
    ...
    if (enable_output_key_checker)
    {
        ...
        ([&](const crow::request& req) {
            return litblocks.show_rawoutputkeys();
        });
        ...
        ([&](const crow::request& req) {
            ...
            return litblocks.show_checkcheckrawoutput(raw_data, viewkey);
        });
    }


    CROW_ROUTE(app, "/paieska").methods("GET"_method)
    ([&](const crow::request& req) {
        return litblocks.search(remove_bad_chars(string(req.url_params.get("value"))));
    });

    CROW_ROUTE(app, "/mempool")
    ([&](const crow::request& req) {
        return litblocks.mempool(true);
    });
    ...
    if (enable_json_api)
    {
        ...
        ([&](const crow::request &req, string tx_hash) {
            ...
            mylit::jsonresponse r{litblocks.json_transaction(remove_bad_chars(tx_hash))};
        ...
        ([&](const crow::request &req, string tx_hash) {
            ...
            mylit::jsonresponse r{litblocks.json_rawtransaction(remove_bad_chars(tx_hash))};

            return r;
        });

        CROW_ROUTE(app, "/api/blokas/<string>")
        ([&](const crow::request &req, string block_no_or_hash) {

            mylit::jsonresponse r{litblocks.json_block(remove_bad_chars(block_no_or_hash))};

            return r;
        });

        CROW_ROUTE(app, "/api/rawblokas/<string>")
        ([&](const crow::request &req, string block_no_or_hash) {

            mylit::jsonresponse r{litblocks.json_rawblock(remove_bad_chars(block_no_or_hash))};
        ...
        ([&](const crow::request &req) {
            ...
            mylit::jsonresponse r{litblocks.json_transactions(
        ...
        ([&](const crow::request &req) {
            ...
            mylit::jsonresponse r{litblocks.json_mempool(
                    remove_bad_chars(page), remove_bad_chars(limit))};
        ...
        ([&](const crow::request &req, string search_value) {
            ...
            mylit::jsonresponse r{litblocks.json_search(remove_bad_chars(search_value))};
        ...
        ([&](const crow::request &req) {
            ...
            mylit::jsonresponse r{litblocks.json_networkinfo()};
        ...
        ([&](const crow::request &req) {
            ...
            mylit::jsonresponse r{litblocks.json_emission()};
        ...
        ([&](const crow::request &req) {
            ...
            mylit::jsonresponse r{litblocks.json_outputs(
        ...
        ([&](const crow::request &req) {
            ...
            mylit::jsonresponse r{litblocks.json_outputsblocks(
        ...
        ([&](const crow::request &req) {
            ...
            mylit::jsonresponse r{litblocks.json_version()};
    ...
    if (enable_autorefresh_option)
    {
        ...
        ([&]() {
            ...
            return litblocks.index(page_no, refresh_page);
        });
    }
    ...
    if (use_ssl)
    {
        ...
        app.port(app_port).ssl_file(ssl_crt_file, ssl_key_file, ssl_chain_file)
                .multithreaded().run();
    }
    else
    {
    ...
    if (enable_emission_monitor == true)
    {
        ...
        liteg::CurrentBlockchainStatus::m_thread.interrupt();
        liteg::CurrentBlockchainStatus::m_thread.join();
    ...
    liteg::MempoolStatus::m_thread.interrupt();
    liteg::MempoolStatus::m_thread.join();
